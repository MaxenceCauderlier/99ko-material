...
<form method="post" action="<?php echo $runPlugin->getPublicUrl(); ?>send.html">
  ...
  <p>
    ...
    <textarea required="required" name="message" class="materialize-textarea"><?php echo $message; ?></textarea>
  </p>
  <?php if($acceptation){ ?>
  <p class="acceptation">
      <label>
          <input type="checkbox" required="required" /><span> <?php echo $runPlugin->getConfigVal('acceptation'); ?></span>
      </label>
  </p>
  <?php } ?>
